"use client";
import { IImage, IRoom } from "@/server/models/room";
...
const UploadRoomImages = ({ data }: Props) => {
  ...
  const removeImagePreview = (imgUrl: string) => {
    const filteredImagesPreview = imagesPreview.filter((img) => img != imgUrl);  //new preview arr after removing passed imgurl
